...
public class CodeListSpecification {

    private static final Logger logger = LoggerFactory.getLogger(CodeListSpecification.class);

    public static Specification<CodeList> applyFiltersAndSort(List<Filter> filters, List<SortCriteria> sortCriteria) {
        return (root, query, criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();
            logger.info("Applying {} filters and {} sort criteria for CodeList", filters.size(), sortCriteria.size());
                        ...
                        case EQUALS:
                            if (fieldName.equals("creationDate")) {
                                predicates.add(criteriaBuilder.equal(
                                        path, LocalDateTime.parse(value)));
                            } else {
